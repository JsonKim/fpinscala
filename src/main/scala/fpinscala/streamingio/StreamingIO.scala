...
object SimpleStreamTransducers {
  ...
  object Process {
    ...
    def take[I](n: Int): Process[I,I] = {
      def go(n: Int): Process[I,I] = Await[I,I] {
    ...
    def take2[I](n: Int): Process[I,I] =
      if (n > 0) await(i => emit(i, take2[I](n-1)))
      else Halt()

    def drop[I](n: Int): Process[I,I] =
      if (n > 0) await(i => drop[I](n-1))
      else id
    ...
    def mean: Process[Double,Double] = {
      def go(acc: Double, n: Int): Process[Double,Double] =
        await(d => emit(d+acc/n+1, go(d+acc, n+1)))
    ...
    def sum2: Process[Double,Double] = loop(0.0)((x, acc) => (x+acc, x+acc))

    def count3[I]: Process[I,Int] = loop(0)((_, n) => (n+1, n+1))
